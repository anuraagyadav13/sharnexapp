...
const getDatesOfWeek = (pivot: Date) => {
      ...
      <TouchableOpacity activeOpacity={1} style={styles.calModalOverlay} onPress={onClose}>
        <Animated.View entering={FadeInUp.springify()} style={styles.calContainer}>
          <View style={styles.calHeader}>
            <TouchableOpacity onPress={onClose}><Ionicons name="close" size={24} color={theme.subtext} /></TouchableOpacity>
...
const ScheduleCard = ({ item, index }: { item: any, index: number }) => {
  ...
  if (isBreak) {
    return (
      <Animated.View entering={FadeInDown.delay(index * 100).springify()} style={[styles.card, styles.freeCard]}>
        <MaterialCommunityIcons name="coffee-outline" size={20} color={theme.subtext} />
  ...
  if (isFree) {
    return (
      <Animated.View entering={FadeInDown.delay(index * 100).springify()} style={[styles.card, styles.freeCard]}>
        <MaterialCommunityIcons name="clock-outline" size={20} color={theme.subtext} />
  ...
  return (
    <Animated.View entering={FadeInDown.delay(index * 100).springify()} style={[styles.card, isSubstitution && styles.substitutionCard]}>
      <View style={styles.cardHeader}>
        ...
        <TouchableOpacity style={{ padding: 6 }}><Ionicons name="ellipsis-horizontal" size={18} color={theme.subtext} /></TouchableOpacity>
      </View>
      <View style={styles.cardFooter}>
        <Ionicons name="person-outline" size={12} color={theme.subtext} style={{ marginRight: 6 }} />
...
const ScheduleRow = ({ item, index }: { item: any, index: number }) => {
  ...
  return (
    <Animated.View entering={FadeInDown.delay(index * 80).springify()} style={styles.tableRow}>
      ...
      <TouchableOpacity style={styles.colAction}><Ionicons name="open-outline" size={16} color={theme.border} /></TouchableOpacity>
    </Animated.View>
  );
};
...
const getStyles = (theme: any) => StyleSheet.create({
  ...
  subtitle: { fontSize: 11, fontWeight: '600', color: theme.subtext },
  content: { flex: 1 },
  ...
  timeTag: { fontSize: 10, fontWeight: '700', color: theme.subtext, marginRight: 8 },
  statusTag: { paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4 },
  statusTagText: { fontSize: 7, fontWeight: '900' },
  subjectText: { fontSize: 15, fontWeight: '900', color: theme.text },
  classText: { fontSize: 11, color: theme.subtext, fontWeight: '600', marginTop: 1 },
  cardFooter: { flexDirection: 'row', alignItems: 'center', marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: theme.border },
  footerText: { fontSize: 10, fontWeight: '700', color: theme.subtext },

  tableView: { backgroundColor: theme.surface, borderRadius: 16, borderWidth: 1, borderColor: theme.border, padding: 10 },
  tableHead: { flexDirection: 'row', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.border, marginBottom: 8 },
  tableHeadText: { fontSize: 9, fontWeight: '800', color: theme.subtext, letterSpacing: 0.5 },
  ...
  lLine: { flex: 1, height: 1, backgroundColor: theme.border },
  lMark: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12 },
  lText: { fontSize: 9, fontWeight: '900', color: theme.subtext, letterSpacing: 1 },
  summaryRow: { flexDirection: 'row', marginBottom: 16, gap: 10 },
  sumCard: { flex: 1, borderRadius: 10, padding: 12, borderWidth: 1, borderColor: theme.border, elevation: 2, backgroundColor: theme.surface },
  sumLabel: { fontSize: 8, fontWeight: '800', color: theme.subtext, marginBottom: 2 },
  sumVal: { fontSize: 16, fontWeight: '900', color: theme.text },
  printBtn: { alignSelf: 'center', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, borderWidth: 1, borderColor: theme.border, marginTop: 10 },
  printBtnText: { fontSize: 11, fontWeight: '700', color: theme.subtext },
  ...
  formCol: { flex: 1 },
  inputLabel: { fontSize: 12, fontWeight: '800', color: theme.text, marginBottom: 8 },
  inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: 10, paddingHorizontal: 12, height: 48 },
  textInputVal: { flex: 1, fontSize: 14, color: theme.text, fontWeight: '600' },
  ...
  calMonthText: { fontSize: 15, fontWeight: '700', color: theme.text },
  calGrid: { flexDirection: 'row', flexWrap: 'wrap' },
  calDayBox: { width: '14.28%', alignItems: 'center', marginBottom: 10 },
  calDayLabel: { fontSize: 10, fontWeight: '800', color: theme.subtext },
