...
const TeacherEquipmentDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleAction = async (action: 'submit' | 'cancel' | 'acknowledge') => {
    try {
      ...
      if (action === 'submit') {
        await teacherService.submitEquipmentRequest(requestId);
      } else if (action === 'cancel') {
        await teacherService.cancelEquipmentRequest(requestId);
      } else if (action === 'acknowledge') {
        await teacherService.acknowledgeEquipmentRequest(requestId, { acknowledgementNote: ackNote });
      }
...
const getStyles = (theme: any) => StyleSheet.create({
  ...
  headerDate: { fontSize: 11, color: theme.subtext, fontWeight: '500' },
  ...
  purposeText: { fontSize: 14, color: theme.text, lineHeight: 22 },
  ...
  noteTitle: { fontSize: 12, fontWeight: '700', color: theme.subtext, marginBottom: 4 },
  noteText: { fontSize: 13, color: theme.text, lineHeight: 18 },
  ...
  itemNote: { fontSize: 11, color: theme.subtext, marginTop: 2 },
  quantityBox: { alignItems: 'flex-end', gap: 2 },
  qtyLabel: { fontSize: 11, fontWeight: '600', color: theme.subtext },
  qtyVal: { color: theme.text, fontWeight: '700' },
  ...
  timelineAction: { fontSize: 13, fontWeight: '700', color: theme.text, textTransform: 'capitalize' },
  timelineDate: { fontSize: 11, color: theme.subtext },
  timelineActor: { fontSize: 12, color: theme.subtext, fontWeight: '500' },
  timelineRemarks: { fontSize: 12, color: theme.text, marginTop: 6, fontStyle: 'italic' },
