...
const PrincipalAddSubjectScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast('Subject successfully registered!', 'success');
      setTimeout(() => navigation.goBack(), 1500);
    } catch (e: any) {
      console.error('Failed to create subject:', e);
      const errorMsg = e.response?.data?.message || 'Unable to register subject. Please verify network connectivity.';
