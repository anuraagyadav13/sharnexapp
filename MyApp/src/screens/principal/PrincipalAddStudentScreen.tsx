  ...
  <View style={styles.field}>
    ...
    <View style={{ flexDirection: 'row', gap: 12 }}>
      ...
      {onPress ? (
        ...
        >
          <Text
            style={[styles.premiumInputText, !value && { color: '#94A3B8' }]}
          >
            {value || placeholder}
          </Text>
...
const PrincipalAddStudentScreen = ({ navigation }: any) => {
  ...
  const handleSelectOption = (option: string) => {
    if (
      selectionConfig.field.includes('countryCode') ||
      selectionConfig.field.includes('CountryCode')
    ) {
      ...
      const selectedClass = classes.find(
        c => c.name === option || c.className === option,
      );
      updateForm(
        'classId',
        selectedClass
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        parentPhoneNumber: formData.guardianPhone
          ...
          <View style={styles.formSection}>
            ...
            <FormField
              ...
              value={
                classes.find(
                  c =>
                    c.id === formData.classId ||
                    c.name === formData.classId ||
                    c.className === formData.classId,
                )?.name ||
                classes.find(
                  c =>
                    c.id === formData.classId ||
              ...
              onPress={() =>
                handleOpenSelection(
                  'Select Class',
                  'classId',
                  classes.map(c => c.name || c.className),
                )
              }
