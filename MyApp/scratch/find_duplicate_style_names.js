...
function findDuplicateStyleNames(filePath) {
    ...
    // Find all style names. A style name is followed by a colon and then an open brace or a single line object
    const styleNameMatches = stylesBlock.matchAll(/^\s+([a-zA-Z0-9_]+):/gm);
    const names = [];
    for (const m of styleNameMatches) {
        const name = m[1];
        if (names.includes(name)) {
            console.log(`DUPLICATE STYLE NAME: '${name}' in ${filePath}`);
        }
        names.push(name);
    }
}
